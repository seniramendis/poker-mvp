 ...
 * cards out.
 */
// Timing for one full "pitch a card" cycle: a brief wind-up (arm draws back
// and the torso coils slightly), a fast snap-throw, and a short follow-
// through before resetting for the next card. Keyframe `times` below line
// up with this so the throw reads as a distinct flick rather than a smooth
// back-and-forth twitch.
const THROW_DURATION = 0.62;
const THROW_TIMES = [0, 0.28, 0.52, 0.72, 1];
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={
          isDealing
            ? { duration: THROW_DURATION, times: THROW_TIMES, repeat: Infinity, ease: 'easeInOut' }
      ...
      >
        ...
        >
          {/* Torso / vest - a faint coil-and-release on the throw so the
              whole body reads as putting weight behind the pitch */}
          <motion.g
            animate={
              isDealing
                ? { rotate: [0, -2.5, 4, 1, 0] }
          ...
          {/* Static resting arm - idle sway, or a light counter-balance
              sway while dealing/shuffling */}
          <motion.g
          ...
          {/* Dealing arm - draws back, snaps forward to pitch a card, then
              follows through before resetting for the next one */}
          <motion.g
          ...
          >
            ...
            {/* The card actually leaving the hand mid-throw */}
            {isDealing && (
              <motion.rect
                x="90"
                y="112"
                ...
                style={{ transformOrigin: '97px 121px' }}
              />
            )}
          </motion.g>
          ...
          {/* Head - a slow, natural side-to-side tilt at idle, settling
              still while actively dealing/shuffling */}
          <motion.g
            animate={active ? { rotate: 0 } : { rotate: [0, -2, 0, 2, 0] }}
            transition={active ? { duration: 0.3 } : { duration: 5.5, repeat: Infinity, ease: 'easeInOut' }}
          ...
          >
            ...
            <motion.g
              ...
              transition={{ duration: 4.2, repeat: Infinity, times: [0, 0.92, 0.95, 0.98, 1], ease: 'easeInOut' }}
